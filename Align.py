...
def return_symptom_list(symptoms):
    ...
    for i in zip(symptoms['word']):
        ...
        words.sort()
        m = Matcher(words)
        docs = list(find_all_matches(i[0], 0, m))
        docs += list(find_all_matches(i[0], 1, m))
        if docs:
            return_list.append([v for k, v in df.loc[df['非标准词'] == docs[0]]['标准词'].items()][0])
    return return_list
...
if __name__ == '__main__':
    q = '我有发热、窒息感、水肿怎么办？'
    print(one_line(q))
    print(return_symptom_list(one_line(q)))